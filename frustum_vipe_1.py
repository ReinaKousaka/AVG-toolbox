...
def _compute_pw_sparse_for_all(
    ...
    uu_s, vv_s = np.meshgrid(cs + 0.5 * point_stride, rs + 0.5 * point_stride)
    scale_x = W_img / Wd
    scale_y = H_img / Hd
    ...
    for n in tqdm(range(T), desc="pre-sparse"):
        z_full = (
            cv2.resize(depths[n], (len(cs), len(rs)), interpolation=cv2.INTER_AREA)
            .astype(float)
            .ravel()
        )
...
def check_depth_overlap_sequence(
    ...
    with tqdm(range(T), desc="overlap-seq(gpu)") as pbar:
        for t in pbar:
            ...
            window_indices = ranged_values(n_hi, n_lo)
            # print(len(window_indices))
...
def process_single_video(
    ...
    if True:
        ...
        if not os.path.exists(intrinsic_path):
            exit()
        dep_path = os.path.join(cam_dir, "depth", name + ".zip")
        ...
        raw_depths = load_depth_zip_to_array(dep_path).astype(np.float32) * float(
            depth_scale
        )
    ...
    grouped_info_dict = grouped_info_dict or {}
    save_depths = []
    w_frame = depths.shape[2]
    h_frame = depths.shape[1]
    for d in depths:
        save_depths.append(
            d[point_stride // 2 :: point_stride, point_stride // 2 :: point_stride]
        )
    save_depths = np.stack(save_depths, axis=0)
    save_depths_dict = {}
    ...
    save_depths_verbose = np.concatenate(
        (
            save_depths_verbose[..., None],
            np.zeros_like(save_depths_verbose[..., None]),
            (255 - save_depths_verbose[..., None]),
        ),
        axis=-1,
    )
