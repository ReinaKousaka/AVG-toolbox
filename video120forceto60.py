...
def needs_reencode_to_60fps(fps: float | None) -> bool:
    """
    判断是否需要转为 60fps。
    允许一定误差（例如 29.97）。
    """
    if fps is None:
        return True
    return not (59.9 <= fps <= 60.1)

def transcode_to_60fps(src: Path, dst: Path, crf: int = 20, preset: str = "medium"):
    """
    使用 fps 滤镜把视频统一到 60fps；视频用 libx264 重编码，音频拷贝。
...
def maybe_copy(src: Path, dst: Path):
    """
    直接拷贝（已是 ~60fps 的情况）。也可以改成“重封装为 mp4”以统一容器。
    这里保持统一输出容器为 mp4，因此即使已是 60fps，我们也做一次“零变化”重封装：
      -c:v copy -c:a copy
    """
    cmd = [
        "ffmpeg", "-y",
...
def main():
    parser = argparse.ArgumentParser(description="把文件夹下视频统一为 60fps（时长不变），保存到新文件夹并清理文件名。")
    ...
    parser.add_argument("--force-reencode", action="store_true",
                        help="即使已是60fps也强制转码（默认遇到60fps仅重封装拷贝）")
    ...
    for src in sorted(videos):
        ...
        fps = get_avg_fps(src)
        try:
            if args.force_reencode or needs_reencode_to_60fps(fps):
                transcode_to_60fps(src, dst, crf=args.crf, preset=args.preset)
            else:
                # 已是 ~60fps：仅重封装为 mp4，避免重编码
                maybe_copy(src, dst)
